...
SELECT
    t.city,
    t.cnt,
    round(t.cnt*100/44744.0, 2) as percentage
FROM t
ORDER BY t.cnt DESC
LIMIT 10
...
UNION ALL
(SELECT
    'Total programmers' Всего,
    count(*)::text
FROM hh.candidate cnd                   /* Работаем с таблицей кандидатов... */
JOIN hh.city ct ON ct.id = cnd.city_id  /* .. соединённой с таблицей городов по ID города */
WHERE                                   /* Условия отбора кандидатов */
--    ct.title = 'Москва'                 /* .. в Москве */
    (                               /* .. и желательная должность.. */
        lower(cnd.desirable_occupation) like '%разработчик%' /* .. разработчик */
        OR                                                   /* или */
        lower(cnd.desirable_occupation) like '%аналитик%'    /* .. аналитик */
        OR                                                   /* или */
        lower(cnd.desirable_occupation) like '%программист%' /* .. программист */
        OR                                                   /* или */
        lower(cnd.desirable_occupation) like '% it%' /* .. IT */
        OR                                                   /* или */
        lower(cnd.desirable_occupation) like '%sql%' /* .. программист */
        OR                                                   /* или */
        lower(cnd.desirable_occupation) like '%software%' /* .. программист */
        OR                                                   /* или */
        lower(cnd.desirable_occupation) like '%developer%' /* .. программист */
        OR                                                   /* или */
        lower(cnd.desirable_occupation) like '%devops%' /* .. программист */
        OR                                                   /* или */
        lower(cnd.desirable_occupation) like '%java%' /* .. программист */
        OR                                                   /* или */
        lower(cnd.desirable_occupation) like '%c++%' /* .. программист */
        OR                                                   /* или */
        lower(cnd.desirable_occupation) like '%c#%' /* .. программист */
        OR                                                   /* или */
        lower(cnd.desirable_occupation) like '%php%' /* .. программист */
    )
)
...
SELECT
    count(cnd.id) cnt,                  /* Считаем кандидатов */
    ct.title city                       /* Добываем название города из таблицы Городов */
FROM hh.candidate cnd                   /* Работаем с таблицей кандидатов... */
JOIN hh.city ct ON ct.id = cnd.city_id  /* .. соединённой с таблицей городов по ID города */
WHERE                                   /* Условия отбора кандидатов */
    cnd.current_occupation = cnd.desirable_occupation /* занимаемая должность совпадает с желаемой */
GROUP BY ct.title              /* группируем результат выборки по городу и потом по ID кандидита */
ORDER BY cnt DESC
LIMIT 10
...
--ответ: 75
SELECT
    count(cnd.age)                      /* Считаем общее количество кандидитов (по колонке Возраст) */
FROM hh.candidate cnd                   /* Работаем с таблицей кандидатов... */
--ответ: 44744

SELECT
    avg(cnd.age)                        /* Считаем средний возраст кандидитов (по колонке Возраст) */
FROM hh.candidate cnd                   /* Работаем с таблицей кандидатов... */
--ответ: 32.19674
...
WHERE                                    /* отбираем в конечный результат только тех кандидитов которым... */
    lower(ttt.title) like '%вахтовый метод%' /* подходит работать вахтовым методом */
GROUP BY city, ttt.title
ORDER BY cnt DESC, city ASC;           /* сортируем городам и по ID кандидата в порядке возрастания */